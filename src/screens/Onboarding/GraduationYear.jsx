...
export const GraduationYear = ({navigation}) => {
  const {currentUserProfile} = useSelector((state) => state.profile);
  const [selection, setSelection] = useState('');
  useValueChange(selection, 'ugGraduationYear');
  const handleChange = (value) => {
    setSelection(value);
    currentUserProfile.degree === 'Current PG'
      ? navigation.navigate('PGDegree')
      : navigation.navigate('CareerInterest');
  };
  return (
    <View style={styles.container}>
      <View style={styles.titleContainer}>
        <Text style={{fontSize: 24}}>In which year will you graduate?</Text>
      </View>
